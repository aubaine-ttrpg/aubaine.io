...
**Why:** REVIEW_DOCTRINE §16.6, §16.7, and §16.8. Agents modify files opportunistically and mix cleanup with features, which hides the real risk under churn. A scoped, single-intent change is reviewable, revertible, and forces the reuse search to be explicit rather than assumed.

**Good / Bad:**
```text
# Bad: one PR mixing a feature with a repo-wide reformat and a dependency bump.
feat: add booking export (+ reformat 240 files, bump 11 deps, tweak CI)

# Good: one intent per PR; mechanical changes split out.
feat: add booking export          # behaviour only, lists files + reuse checked
