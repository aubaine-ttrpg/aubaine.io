...
**Why:** REVIEW_DOCTRINE §16.5. These patterns are how generic generated code reads; each is individually defensible but collectively signals that the change was not integrated. Naming them as smells lets a reviewer scrutinize the right places instead of either rubber-stamping or reflexively rejecting.
...
final class ArchiveExpiredBookings {
    public function __invoke(BookingId $id): void { /* explicit, fail-fast */ }
}
```
