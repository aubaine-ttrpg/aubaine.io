...
and `.env.local`** so a value's home is obvious and diffs stay aligned (the `athletis-testing` skill and
`tests/CONVENTIONS.md` rely on this). One concern per section; do not dump entries above the first
header.
