...
<?php
namespace App\Book;
function ordinalAfter(int $ordinal, int $step): int { return $ordinal + $step; }
ordinalAfter("5", 1); // works, returns 6, masks a real type error
```
```php
...
namespace App\Book;
function ordinalAfter(int $ordinal, int $step): int { return $ordinal + $step; }
ordinalAfter("5", 1); // TypeError: caught immediately at the call site
```
